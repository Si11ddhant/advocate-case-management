...
export const Calendar: React.FC = () => {
  ...
  const upcomingHolidays = COURT_HOLIDAYS.filter(hol => parseLocalDate(hol.date) >= todayVal)
  ...
  const visibleGroups = groupedHearingCases
    .map(group => ({
      ...group,
      visibleCases: group.casesList.filter(c => c.status !== 'Completed'),
      hiddenCases: group.casesList.filter(c => c.status === 'Completed')
    }))
    .filter(group => !group.isPast && (group.visibleCases.length > 0 || group.holiday));

  const hiddenGroups = groupedHearingCases
    .map(group => ({
      ...group,
      hiddenCases: group.casesList.filter(c => c.status === 'Completed'),
      visibleCases: group.casesList.filter(c => c.status !== 'Completed')
    }))
    .filter(group => group.isPast || group.hiddenCases.length > 0)
    .sort((a, b) => parseLocalDate(a.date).getTime() - parseLocalDate(b.date).getTime());
            ...
            <CardContent className="space-y-2.5 max-h-[300px] overflow-y-auto scrollbar-hide pr-1">
              {upcomingHolidays.length === 0 ? (
                <div className="text-center py-8 text-muted-foreground">
                  <AlertTriangle className="mx-auto h-8 w-8 mb-3 text-rose-500" />
                  <p className="text-sm font-semibold">No upcoming court holidays found.</p>
                  <p className="text-xs text-muted-foreground mt-1">All listed holidays have already passed.</p>
                </div>
              ) : (
                upcomingHolidays.map(hol => {
                  ...
                  return (
                    <div key={hol.date} className="flex items-center justify-between p-2.5 border rounded-lg transition-colors text-left text-xs font-bold border-border bg-card/60 hover:bg-muted/10">
                      ...
                      <div className="text-right">
                        <span className="text-[11px] font-extrabold text-primary">{formatted}</span>
          ...
          <CardContent className="space-y-4">
            {visibleGroups.length === 0 ? (
              <div className="text-center py-12 text-muted-foreground">
                ...
                <p className="text-sm font-semibold">No upcoming hearings on the docket.</p>
              </div>
            ) : (
              <div className="space-y-8 text-left border-l border-border/70 pl-4 ml-2">
                ...
                {hiddenGroups.length > 0 && (
                  <div className="mt-3 rounded-3xl border border-border/80 bg-muted/70 p-4">
                    <button
                      type="button"
                      onClick={() => setShowHiddenGroups(prev => !prev)}
                      className="w-full flex items-center justify-between px-4 py-3 rounded-2xl bg-card border border-border/70 text-sm font-bold text-foreground hover:bg-muted/90 transition-all"
                    >
                      <span>{showHiddenGroups ? 'Hide past/completed records' : `Show past/completed records (${hiddenGroups.length})`}</span>
                      <ChevronRight className={`transition-transform ${showHiddenGroups ? 'rotate-90' : ''}`} size={18} />
                    </button>
                    ...
                    {showHiddenGroups && (
                      <div className="mt-4 space-y-4">
                        {hiddenGroups.map(group => {
                          ...
                          return (
                            <div key={`hidden-${group.date}`} className="space-y-3 rounded-2xl border border-border/70 bg-card/70 p-3">
                              <div className="flex flex-wrap items-center gap-2">
                                <h3 className="text-xs font-black text-foreground font-mono uppercase tracking-wider">{formatted}</h3>
                                <span className="text-[10px] font-extrabold px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-500 border border-rose-500/20">
                                  {group.holiday ? 'Holiday / Past' : 'Past / Completed'}
                                </span>
                              </div>
                              ...
                              {group.hiddenCases.map(c => (
                                <div key={`hidden-case-${c.id}`} className="rounded-2xl border border-border/60 bg-muted/40 p-3 text-sm text-foreground">
                                  <div className="flex items-center justify-between gap-3">
                                    <div className="min-w-0">
                                      <p className="font-semibold truncate">{c.case_title}</p>
                                      <p className="text-[11px] text-muted-foreground truncate">{c.court_name}</p>
                                    </div>
